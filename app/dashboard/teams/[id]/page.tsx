        ...
        <div className="max-w-[400px] mx-auto">
          <div className="flex items-center gap-2 justify-center mt-8 border-2 rounded-md w-3/4 md:w-1/2 mx-auto py-1">
            <Image
              ...
              style={{ width: "auto", height: "auto" }}
            />
            <h4 className="text-md pr-2">{actualTeam?.country?.countryName}</h4>
          </div>
        </div>
        {actualTeam?.yearsOfChampionship &&
        actualTeam?.yearsOfChampionship.every((it) => it.length > 1) ? (
          <div className="flex flex-col w-full items-center gap-2 justify-center mt-4">
            ...
            <div className="flex flex-col gap-4 w-1/2 items-center mt-4">
              {actualTeam?.yearsOfChampionship.map((it) => (
                <div
                  className="border-2 rounded-md flex items-center gap-2 px-2 py-1"
                  key={it}
                >
                  <GiCheckMark className="text-emerald-500" />
          ...
          <div className="mt-8 flex justify-center gap-2 items-center">
            {" "}
            <RxCrossCircled className="text-red-500" /> No ganó ninguna edición
            de la Copa
          </div>
        )}
      </div>
